...
One practical constraint is that TensorRT selects kernels and memory layouts optimized for the specific GPU it builds on. GPUs within the same generation (e.g.\ RTX 4080 Super and RTX 4060, both Ada Lovelace) share compute capability 8.9, so an engine built on one can run on the other, but rebuilding on the target hardware ensures optimal kernel selection. We therefore built the final engine directly on the race car's RTX 4060.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.5\textwidth]{racecar.png}
