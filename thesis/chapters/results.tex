...
\begin{figure}[ht]
\centering
\includegraphics[width=1\textwidth]{comparison_lidar1_left_gt_detections.jpg}
...
On the RTX 4060 onboard the race car, the YOLO26n TensorRT FP16 engine averages 2.63~ms per image in isolated benchmarks (\texttt{trtexec}: 1.58~ms host-to-device, 1.02~ms GPU compute, 0.03~ms device-to-host). The production YOLO11n engine is comparable at 2.70~ms; both models are transfer-bound rather than compute-bound at this scale. Note that model inference is only one stage of the full perception pipeline---preprocessing, stereo matching, feature matching, and triangulation add substantial overhead. As shown in \cref{sec:integration}, the complete pipeline runs at 15.61~ms for YOLO26n, just within the 16.7~ms budget for 60~fps. We did not pursue INT8 quantization as the 1--2\% \mAPfifty{} penalty is not justified when inference itself is not the bottleneck.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.4\textwidth]{yolo26_occlusion.jpg}
