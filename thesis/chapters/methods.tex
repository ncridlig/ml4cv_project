...
All training scripts, configurations, and evaluation code are publicly available \citep{cridlig2026code}. We used a M1 MacBook Air for code development and an Ubuntu workstation with an RTX 4080 Super for training. The existing inference pipeline (\texttt{ubm-yolo-detector}) enforced the use of Ultralytics models, which we kept to slot the new model into the existing modular framework. Weights and Biases \citep{wandb} provided experiment tracking. The Autonomous System Unit (ASU) on the race car is a water-cooled desktop running ROS2 \citep{fusa2025}; for our purposes we care only about its GPU, an RTX 4060.
