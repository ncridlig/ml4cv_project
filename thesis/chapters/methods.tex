...
All training scripts, configurations, and evaluation code are publicly available \citep{cridlig2026code}. We used a M1 MacBook Air for code development and an Ubuntu workstation with a RTX 4080 Super for training. The existing inference pipeline (\texttt{ubm-yolo-detector}) enforced the use of Ultralytics models, which we kept to slot the new model into the existing modular framework. Weights and Biases \citep{wandb} provided experiment tracking. The Autonomous System Unit (ASU) on the race car is a water-cooled desktop running ROS2 \citep{fusa2025}; for our purposes we care only about its GPU, a RTX 4060.

\begin{figure}[ht]
\centering
\includegraphics[width=0.75\textwidth]{ASU.jpg}
\caption{The Autonomous System Unit (white box) benchmarking YOLO models.}
\label{fig:asu}
\end{figure}
...
ASU (Race Car)       & Deployment                & RTX 4060 (CUDA)           & Ubuntu (ROS2)  \\ \bottomrule
\end{tabular}
\end{table}
...
We used three datasets, summarized in \cref{tab:datasets}.
...
Since hyperparameter tuning proved ineffective, we evaluated whether newer architectures could break through the YOLO11n performance ceiling. We trained YOLO12n \citep{yolo12} and YOLO26n \citep{yolo26} under identical conditions: FSOCO-12 dataset, 300 epochs, batch 64, Ultralytics default hyperparameters. All three models are nano variants with comparable parameter counts (2.51--2.59M) and GFLOPs (5.8--6.4), as shown in \cref{tab:architectures}, making the comparison fair with respect to model capacity. YOLO12n finished at \mAPfifty{}=0.708 on the test set, a marginal gain over YOLO11n (0.707). YOLO26n reached \mAPfifty{}=0.763, a substantial 7.9\% improvement over our baseline and 14.6\% over the UBM production model. Every class improved, with the largest absolute gains on unknown cones (+23.4\%) and orange cones (+6.8\%). 
\begin{table}[ht]
\centering
...
YOLO26n         & 2.51                  & 5.8               & 2026          \\ \bottomrule
\end{tabular}
\end{table}
